...
volatile u32 duration_injecting[] =
{
	450,
	250,
	300
...
volatile i32 current_injecting[] =
{
	7000,
	15000,
	2750
...
namespace Nhk23Servo
{
	...
	void inject_callback(const ReceivedMessage& message) noexcept
	{
		...
		current = current_injecting[static_cast<Command>(message.data.buffer[0])];
		duration = duration_injecting[static_cast<Command>(message.data.buffer[0])];
		time = HAL_GetTick();
	}
}
